...
def cluster_tracks_2d(rgb_seq, tracks_2d, use_diff=False, visualize=False):
    """
    ...
    if visualize:
        ...
        vis_tracks2d_napari(rgb_seq, tracks_2d, colors=rigid_part_colors)
        
    return moving_mask, static_mask


def cluster_tracks_3d(tracks_3d, use_diff=False, visualize=False):
    """
        pred_tracks_3d: torch.tensor([T, M, Nd])
        将 pred_tracks_3d 进行聚类，得到两个部分，分别代表移动和静止的轨迹, 返回两个 mask
    """
    pred_tracks_3d = tracks_3d.cpu()
    T, M, _ = pred_tracks_3d.shape
    
    if use_diff:
        # 减去初始帧, 也就是用 difference 作为 clustering 的输入
        pred_tracks_3d = pred_tracks_3d - pred_tracks_3d[0:1, ...]
        
    motion_for_kmeans = pred_tracks_3d.permute(1, 0, 2).reshape(M, -1) # M T*Nd
    _, moving_labels, _ = cluster.k_means(motion_for_kmeans.numpy(), init="k-means++", n_clusters=2)

    # 确定哪一类是 moving, 那一类是 static, 依据是轨迹的 variance
    part1 = pred_tracks_3d[:, moving_labels == 0, :] # T, M1, Nd
    part2 = pred_tracks_3d[:, moving_labels == 1, :] # T, M2, Nd
    ...
    if visualize:
        rigid_part_colors = np.zeros((pred_tracks_3d.shape[1], 3)) # M, 3
        rigid_part_colors[moving_mask] = np.array([1, 0, 0]) # red
        rigid_part_colors[static_mask] = np.array([0, 0, 1])
        vis_tracks3d_napari(tracks_3d, colors=rigid_part_colors)
