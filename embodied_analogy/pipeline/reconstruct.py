...
def reconstruct(
    ...
    obj_mask_seq = whole_obj_masking_sam(
        ...
        franka_seq[informative_frame_idx], 
        visualize
    ) 
    ...
    obj_mask_seq = obj_mask_seq & depth_mask_seq[informative_frame_idx]
    """
        根据 tracks2d 和 obj_mask_seq 得到 dynamic_seq
    """
    dynamic_seq = get_dynamic_mask_seq(
        obj_mask_seq, 
    ...
    if save_dir is not None:
        np.savez(
            ...
            joint_type=joint_type,
            franka_seq=franka_seq[informative_frame_idx],
        )
