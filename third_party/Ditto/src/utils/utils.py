...
def save_config(config: DictConfig) -> None:
    ...
    """
    config_saving_path = f"{os.getcwd()}/config.yaml"
    OmegaConf.save(config=config, f=config_saving_path)


@rank_zero_only
def save_results(config_d: Dict) -> None:
    """
    save config to experiment dir
    """
    config = OmegaConf.create(config_d)
    config_saving_path = f"{os.getcwd()}/results.yaml"
    OmegaConf.save(config=config, f=config_saving_path)
